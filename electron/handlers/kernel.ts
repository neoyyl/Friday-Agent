...
export function registerKernelHandlers(registry: ServiceRegistry): void {
  const svc = {
  ...
  ipcMain.handle('kernel:getStderrLog', () => {
    return { log: [] }
  })

  // --- Generic proxy (routes common API paths to native services) ---
  ...
  ipcMain.handle('kernel:voice:register', async (_event, name: string, config?: any) => {
    try { return success(svc.voice()?.register(name, config)) } catch (e) { return wrapError(e) }
  })

  ipcMain.handle('kernel:voice:deleteSpeaker', async (_event, name: string) => {
    try { return success(svc.voice()?.deleteSpeaker(name)) } catch (e) { return wrapError(e) }
  })

  ipcMain.handle('kernel:voice:currentSpeaker', async () => {
  ...
  // --- ASR ---
  ipcMain.handle('kernel:asr:transcribe', async (_event, audioBase64: string, lang?: string) => {
